...
class LineByLineTextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path_src, file_path_tgt, gold_path):
        ...
        if args.cache_data and os.path.isfile(cache_fn) and not args.overwrite_cache:
            ...
            i = 0
            for line_id, (line_src, line_tgt) in tqdm(
                enumerate(zip(lines_src, lines_tgt))
            ):
                i = i + 1
                if line_src and line_tgt:
                    sent_src, sent_tgt = (
                        line_src.strip().split(),
                        line_tgt.strip().split(),
                    )
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if (
                    ...
                    output_dir_adapter = os.path.join(
...
def evaluate(args, model, tokenizer, global_step, prefix="") -> Dict:
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        with torch.no_grad():
            ...
            word_aligns_list_batch = model.get_aligned_word(
                inputs_src,
                inputs_tgt,
                bpe2word_map_src,
                bpe2word_map_tgt,
...
def main():
    ...
    parser.add_argument(
        ...
        default="",
        type=str,
        help="The input evluating data file (a text file).",
    )
    parser.add_argument(
        "--eval_data_file_tgt",
        default="",
        type=str,
        help="The input evaluating data file (a text file).",
    )
    ...
    parser.add_argument(
        "--align_layer", type=int, default=6, help="layer for alignment extraction"
    )
    parser.add_argument(
        "--extraction",
        default="softmax",
    ...
    parser.add_argument("--softmax_threshold", type=float, default=0.1)
    ...
    parser.add_argument(
        "--do_test", action="store_true", help="Whether to run infer on the dev set."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--reduction_factor", type=int, default=6, help="reduction_factor for adapter"
    )
    parser.add_argument(
    ...
    labse_model = AutoModel.from_pretrained(
        args.model_name_or_path, output_hidden_states=True
    )
    adapters.init(labse_model)

    if args.do_train:

        config_align = DoubleSeqBnConfig(reduction_factor=6)
