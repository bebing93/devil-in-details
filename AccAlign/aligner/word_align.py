...
class SentenceAligner_word(object):
    ...
    def get_subword_matrix(
        ...
        output_src, output_tgt = self.embed_loader(
            ...
            attention_mask_src=(inputs_src != PAD_ID).long(),
            attention_mask_tgt=(inputs_tgt != PAD_ID).long(),
            guide=None,
        ...
        if align_layer != -1:
            ...
            layers = range(1, len(output_src.hidden_states))
        for layer_id in layers:
            ...
            # mask
            attention_mask_src = (
                (inputs_src == PAD_ID) + (inputs_src == CLS_ID) + (inputs_src == SEP_ID)
            ).float()
            attention_mask_tgt = (
            ...
            attention_mask_tgt = return_extended_attention_mask(
                1 - attention_mask_tgt, hidden_states_tgt.dtype
            )
            ...
            attention_scores_tgt = attention_scores + attention_mask_src.transpose(
                -1, -2
            )

            attention_probs_src = nn.Softmax(dim=-1)(
                attention_scores_src
            )  # if extraction == 'softmax' else entmax15(attention_scores_src, dim=-1)
            attention_probs_tgt = nn.Softmax(dim=-2)(
                attention_scores_tgt
            )  # if extraction == 'softmax' else entmax15(attention_scores_tgt, dim=-2)
    ...
    def get_aligned_word(
        ...
        attention_probs_inter_all_layers = self.get_subword_matrix(
            args,
            inputs_src,
            inputs_tgt,
            PAD_ID,
