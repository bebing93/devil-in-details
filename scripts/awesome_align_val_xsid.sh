...
for original_lang in ar da de de-st id it kk nl sr tr zh; do
    ...
    TRANSLATED_DATA_FILE=$OUT_DIR/val-translate-${original_lang}-en-tokens-processed.jsonl
    ...
    # We treat the translated data as source in T-Test
    python $WORK_DIR/devil_in_details/alignment/postprocess_alignment_ttest.py $TRANSLATED_ALIGN_IN_FILE $ORIGINAL_DATA_FILE $ALIGNMENT_FILE $DATASET_FILE --tag_name entity_tags
done